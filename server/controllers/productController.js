...
|
| 1. Raw spreadsheet rows:
|    Product Name / Cost Price / Selling Price...
|
| 2. Already-normalized rows returned by /import/preview.
|
| The second case is important because /import/confirm receives
| the preview rows from the frontend.
|
|--------------------------------------------------------------------------
*/
...
const normalizeImportRows = (rows) => {
  return rows.map((row, index) => {
    const rowNumber =
      Number(row.rowNumber) || index + 2;

    /*
    |--------------------------------------------------------------------------
    | Already-normalized preview row
    |--------------------------------------------------------------------------
    */

    const isNormalizedRow =
    ...
    if (isNormalizedRow) {
      ...
      if (
        ...
        errors.push(
          'Tracks Batches must be Yes or No.'
        );
      }
...
const validateImportRows = async ({
  ...
  /*
  |--------------------------------------------------------------------------
  | Duplicate SKUs inside uploaded file
  |--------------------------------------------------------------------------
  */
...
const confirmImport =
  asyncHandler(
    async (req, res) => {
      ...
      /*
      |--------------------------------------------------------------------------
      | Revalidate the preview data.
      |
      | normalizeImportRows now knows that these are already-normalized
      | preview rows, so it will not incorrectly look for "Product Name".
      |--------------------------------------------------------------------------
      */
