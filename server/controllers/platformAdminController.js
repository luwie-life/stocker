  ...
  async (req, res) => {
    ...
    if (req.body.adminResponse) {
      await Notification.create({
        user:
          feedbackItem.submittedBy,
        business:
          feedbackItem.business,
        type: 'FEEDBACK_REPLY',
        title:
...
const inviteSuperAdmin = asyncHandler(
  async (req, res) => {
    const {
      fullName,
      email,
    ...
    if (!fullName || !email) {
      throw new AppError(
        'Full name and email are required to invite a super admin.',
        400,
        'INVALID_INVITE'
      );
    }
    ...
    return ok(res, {
      ...
      temporaryPassword:
        passwordToUse,
      message:
        'Super admin invited successfully. Share the temporary password securely.',
    });
  }
);
...
const saveMarketingContent =
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw new AppError(
        'A title and a valid YouTube link are required.',
        400,
        'INVALID_MARKETING_CONTENT'
      );
    }
...
const ambassadors = asyncHandler(
  async (req, res) => {
    const items =
      await Ambassador.find()
        .populate(
          'user',
          'fullName email phone status createdAt'
        )
        .sort({
          createdAt: -1,
        })
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.body.target) {
      ...
      if (
        ...
        throw new AppError(
          'Target referrals must be a non-negative whole number.',
          400,
          'INVALID_TARGET'
        );
      }
...
const messages = asyncHandler(
  async (req, res) => {
    const items =
      await PlatformMessage.find({
        recipient:
          req.user._id,
      })
        .populate(
          'sender',
          'fullName email platformRole'
        )
        .populate(
          'recipient',
          'fullName email'
...
const sendMessage = asyncHandler(
  async (req, res) => {
    ...
    if (!recipients.length) {
      throw new AppError(
        'No recipients matched this message.',
        404,
        'NO_RECIPIENTS'
      );
    }
...
const businesses = asyncHandler(
  async (req, res) => {
    /*
     * Do not cap this query at 500.
     * Super Admin needs to see every registered
     * business in the platform.
     */
    const items =
      await Business.find()
        .select(
          'name type status subscription createdAt'
        )
        .sort({
          createdAt: -1,
        })
  ...
  asyncHandler(async (req, res) => {
    ...
    if (trialDays !== undefined) {
      ...
      if (
        ...
        throw new AppError(
          'Trial days must be a whole number from 0 to 3650.',
          400,
          'INVALID_TRIAL_DAYS'
        );
      }
    ...
    if (accessDays !== undefined) {
      ...
      if (
        ...
        throw new AppError(
          'Access days must be a whole number from 0 to 3650.',
          400,
          'INVALID_ACCESS_DAYS'
        );
      }
    ...
    business.subscription.updatedBy =
      req.user._id;
